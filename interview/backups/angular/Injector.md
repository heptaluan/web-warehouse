...
转自 [Angular 4.x Injector](https://segmentfault.com/a/1190000009283057)，原文写的不错，但是篇幅很长，内容很深，需要反复进行阅读，主要讲解的是 `Injector` 的源码实现和其中涉及到的一些方法，这里略作调整，整理为自己比较好理解的方式，详细可以参考原文
...
所谓 [依赖注入](https://zh.wikipedia.org/wiki/依赖注入)，本质是一种实现 [控制反转](https://zh.wikipedia.org/wiki/控制反转) 用于解决依赖性设计模式（详细可见对应的维基百科词条），依赖注入允许程序设计遵从依赖倒置原则，调用者只需知道服务的接口，具体服务的查找和创建由注入器（`Injector`）负责处理并提供给调用者，这样就分离了服务和调用者的依赖，符合低耦合的程序设计原则

依赖注入中包含三种角色：调用者、服务和注入器（`Injector`），在 `Angular` 中 `Injector`（注入器） 用来管理服务对象的创建和获取，下面一个一个来看

...
现在，我们可以处理不同的 `URL` 了，但是所有的逻辑都堆在 `app.js` 中，随着 `URL` 越来越多，`app.js` 就会越来越长，我们可以让 `URL` 处理函数集中到某个 `JavaScript` 文件，然后让 `app.js` 自动导入所有处理 `URL` 的函数，这样，代码一分离，逻辑就显得清楚了：
...
`view` 是模版的名称（视图），`model` 就是数据，在 `JavaScript` 中，它就是一个简单的 `Object`，`render` 函数返回一个字符串，就是模版的输出

...
`Redux` 是一套状态管理机制，所谓状态，就是影响 `UI` 变化的数据（`Angular` 中的 `ngrx` 本质上和 `react` 中的 `redux` 那一套是类似的）
...
代码详细地址可见 [index.reducer.ts](https://github.com/hanekaoru/Angular-Task/blob/master/src/app/reducers/index.ts)

首先需要一个 `index.reducer.ts` 来将散布的 `reducer` 进行汇总
...
使用 `effect` 之后，我们就可以来改写上面的例子，将逻辑部分完全的剥离出去

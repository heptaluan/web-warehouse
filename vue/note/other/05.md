...
```js
{
  ...
  // 数组中的最后一个函数必须返回一个字符串或者一个 `ArrayBuffer` || `Stream` || `Buffer` 实例 || `ArrayBuffer` || `FormData`
  ...
  // `paramsSerializer` 是一个可选的函数，是用来序列化参数（https://ww.npmjs.com/package/qs || http://api.jquery.com/jquery.param/)
  ...
  // 在 nodejs 中 `httpAgent` 和 `httpsAgent` 当产生一个 http 或者 https 请求时分别定义一个自定义的代理

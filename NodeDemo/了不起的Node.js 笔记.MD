...
两者的区别在于基础架构上，node采用一个**长期运行的进程**，相反，Apache会产出多个线程（每个请求一个线程），每次都会刷新状态，在PHP中，当解释器再次执行时，变量$book会被重新赋值，而node则不然，serverBooks 函数会再次被调用，且作用域中的变量不受影响（此时$book数组仍为空）

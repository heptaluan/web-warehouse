...
`delete`，`head`，`post`，`put` 和别的 `http` 动作都可以使用，只需要简单的改变方法名称
...
默认发送字符串将设置 `Content-Type` 为 `application/x-www-form-urlencoded`，多个请求将使用 `&` 连接，这里结果是 `name=tj&pet=tobi`
...
`superagent` 的请求数据格式化是可以扩展的，不过默认支持 `form` 和 `json` 两种格式，想发送数据以 `application/x-www-form-urlencoded` 类型的话，则可以简单的调用 `.type()` 方法传递 `form` 参数就行，这里默认是 `json`，下面的请求将会 `postname=tj&pet=tobi` 内容
...
`Node` 客户端通过 `Formidable` 模块支持 `multipart/form-data`，当解析 `multipart` 返回时，对象 `res.files` 对你也是可用的，假设例如一个请求响应如下 `multipart` 结构
...
`res.text` 包含未解析前的响应内容，一般只在 `mime` 类型能够匹配 `text/`，`json`，`x-www-form-urlencoding` 的情况下，默认为 `nodejs` 客户端提供，这是为了节省内存，因为当响应以文件或者图片大内容的情况下影响性能
...
`res.header` 包含解析之后的响应头数据，键值都是 `node` 处理成小写字母形式，比如 `res.header['content-length']`
